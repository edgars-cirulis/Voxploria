...
void WorldRenderer::init(GLFWwindow* w, int width, int height) {
  window = w;
...
void WorldRenderer::drawWorld(float dt) {
  auto ext = vk.getExtent();
  if (ext.height != 0) cam.aspect = float(ext.width) / float(ext.height);
