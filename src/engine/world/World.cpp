...
void World::set(int wx, int wy, int wz, Voxel v) {
  ...
  if (lx == 0) getOrCreate({cc.cx - 1, cc.cz})->dirtyCPU = true;
  if (lx == CHUNK_X - 1) getOrCreate({cc.cx + 1, cc.cz})->dirtyCPU = true;
  if (lz == 0) getOrCreate({cc.cx, cc.cz - 1})->dirtyCPU = true;
  if (lz == CHUNK_Z - 1) getOrCreate({cc.cx, cc.cz + 1})->dirtyCPU = true;
}
